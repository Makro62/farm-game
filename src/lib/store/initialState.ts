...
function createMiningNodes(count = 30): MiningNode[] {
  return Array.from({ length: count }, (_, i) => ({
    id: i,
    status: "ready",
...
function createDefaultWorker(type: WorkerRole | string): Worker | null {
  const templates: Record<string, Worker> = {
    farmer: {
      name: "Kurcaci Budi",
      role: "farmer",
      level: 1,
...
export { createEmptyPlots, createMiningNodes, createDefaultWorker };
