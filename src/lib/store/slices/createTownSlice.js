...
export const createTownSlice = (set, get) => ({
  ...
  updateMarket: () => {
    const newPrices = {};
    const newTrend = {};
    const crops = ['wortel', 'jagung', 'tomat', 'stroberi', 'semangka', 'jamur', 'nanas', 'labu', 'kentang', 'gandum', 'tebu', 'tulip', 'apel'];
  ...
  buyBuilding: (buildingId) => {
    ...
    const state = get();
    if (state.buildings?.[buildingId]) return { ok: false, message: `${building.name} sudah dimiliki.` };
    if (!get().spendCoins(building.price)) return { ok: false, message: 'Koin tidak cukup!' };

    const mineralReq = { silo: { batu: 20, besi: 10 }, greenhouse: { batu: 30, tembaga: 15, emas: 5 } }[buildingId];
    if (mineralReq) {
  ...
  buyDecoration: (decorId) => {
    ...
    if (owned.includes(decorId)) return { ok: false, message: `${decor.name} sudah dimiliki.` };
    if (!get().spendCoins(decor.price)) return { ok: false, message: 'Koin tidak cukup!' };
  ...
  giveGift: (npcId, itemId, isLiked) => {
    ...
    if (!itemCategory || (state.inventoryByCategory?.[itemCategory]?.[itemId]?.qty || 0) <= 0) return null;
