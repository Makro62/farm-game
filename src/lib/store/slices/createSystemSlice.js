import { getMiningRegenMs, isWorkerActive, getGrowthMultiplier, normalizePlot, normalizePlots, normalizeAnimal, pickAutoSeed, safeCoins, safePositiveNumber, getAnimalProduceTime, rollMineralType } from '../utils';
...
export const createSystemSlice = (set, get) => ({
  ...
  openPrompt: (title, msg, onConfirm) => {
    set((state) => ({
      modals: { ...state.modals, prompt: { isOpen: true, title, msg, onConfirm } }
    }));
  },

  openConfirm: (title, msg, onConfirm) => {
    set((state) => ({
      modals: { ...state.modals, confirm: { isOpen: true, title, msg, onConfirm } }
    }));
  },
  ...
  toggleAutoMode: (type) => set(state => {
    const w = state.workers[type];
    if (!w?.hired) return state;
    return { workers: { ...state.workers, [type]: { ...w, isAutoMode: !w.isAutoMode } } };
  }),
  ...
  hireWorker: (type, cost) => {
    ...
    const nameMap = { farmer: 'Kurcaci Budi', rancher: 'Kurcaci Siti', fisher: 'Kurcaci Mamat', miner: 'Kurcaci Tarjo', chef: 'Kurcaci Juna' };
    const skillMap = {
    ...
    set({
      ...
      workers: {
        ...
        [type]: {
          ...
          isWorking: true,
          isAutoMode: true,
          schedule: { workStart: 6, workEnd: 18, lunchBreak: 12, sleepStart: 22, sleepEnd: 5 },
        }
      },
    });
    return true;
  ...
  changeWeather: () => {
    ...
    if (nextChangeIn <= 0) {
      ...
      if (newWeather === '☃️ Bersalju') {
        ...
        set({
          plots: plots.map(p => {
            if (p.crop && p.status === 'growing') return { ...p, status: 'dead', growTime: null };
            return p;
          })
        });
        get().enqueueNotification('Salju turun! Tanaman yang tumbuh menjadi layu ❄️', { icon: '⛄', type: 'info' });
      }
      set({ weather: { current: newWeather, nextChangeIn: 300 }, weatherEffects: effects });
    } else {
  ...
  processGameTick: () => {
    const actions = [
      ...
      () => get().checkOrders(),
      () => { get().tickCustomers(1000); if (Math.random() < 0.1 * (get().weatherEffects?.customerRate || 1)) get().spawnCustomer(); },
      () => {
        const state = get();
        const now = Date.now();
        let changed = false;
        let newCoinMult = state.coinMultiplier;
        if (state.coinMultiplierExpireAt && now > state.coinMultiplierExpireAt) { newCoinMult = 1; changed = true; if (state.coinMultiplier > 1) get().enqueueNotification('Booster Koin telah habis.', { icon: '⏳', type: 'info' }); }
        let newGrowthMult = state.growthMultiplier;
        if (state.growthMultiplierExpireAt && now > state.growthMultiplierExpireAt) { newGrowthMult = 1; changed = true; if (state.growthMultiplier > 1) get().enqueueNotification('Booster Pertumbuhan telah habis.', { icon: '⏳', type: 'info' }); }
        if (changed) {
          set({ coinMultiplier: newCoinMult, growthMultiplier: newGrowthMult, ...(newCoinMult === 1 && { coinMultiplierExpireAt: null }), ...(newGrowthMult === 1 && { growthMultiplierExpireAt: null }) });
        }
      }
    ];
    for (const action of actions) {
      try { action(); } catch (error) { logger.error('Game tick error:', error); }
    }
  },
  ...
  runAutoWorkers: () => {
    ...
    if (isWorkerActive(state, 'farmer')) {
      ...
      for (let i = 0; i < plots.length; i++) {
        ...
        const isReady = p.crop && (p.status === 'ready' || (p.status === 'growing' && p.plantedAt && growTime != null && now - p.plantedAt >= growTime));
        ...
        if (plots[i].status === 'empty') {
          const seedData = pickAutoSeed(state.inventoryByCategory?.seeds || {}, state.selectedSeed, state.season?.current, !!state.buildings?.greenhouse);
          if (seedData) {
            const have = invGet(state, 'seeds', seedData.id);
            if (have > 0) {
              if (!catUpdates['seeds']) catUpdates['seeds'] = {};
              catUpdates['seeds'][seedData.id] = (catUpdates['seeds'][seedData.id] || 0) - 1;
              plots[i] = { id: plots[i].id, status: 'growing', crop: seedData.cropId, plantedAt: now, growTime: (seedData.time * 1000) / growthMult };
              planted++;
              plotsChanged = true;
            }
          }
      ...
      for (let i = 0; i < animals.length; i++) {
        ...
        if (data && a.status === 'producing' && now - a.lastCollected >= produceTime) {
          if (!a.fed) {
            ...
            if (feedData) {
              const have = invGet(state, catFor(feedData.feedItem), feedData.feedItem);
              if (have >= feedData.feedQty) {
                if (!catUpdates[catFor(feedData.feedItem)]) catUpdates[catFor(feedData.feedItem)] = {};
                catUpdates[catFor(feedData.feedItem)][feedData.feedItem] = (catUpdates[catFor(feedData.feedItem)][feedData.feedItem] || 0) - feedData.feedQty;
    ...
    if (plotsChanged || animalsChanged || queueChanged) {
      ...
      if (harvested > 0 || planted > 0) {
        get().enqueueNotification(`👨‍🌾 Kurcaci Budi panen ${harvested} & tanam ${planted}!`, { id: 'auto-farm', type: 'success' });
      }
      if (collected > 0) {
        get().enqueueNotification(`👩‍🌾 Kurcaci Siti ambil ${collected} hasil ternak!`, { id: 'auto-rancher', type: 'success' });
      }
    }
    ...
    if (isWorkerActive(state, 'fisher')) {
      if (Math.random() < GAME_CONSTANTS.CHANCES.FISHER_TICK) {
        ...
        for (const fish of FISHES) {
          ...
          if (rand <= cumulative) { caughtFish = fish; break; }
        }
        set(draft => {
          if (!draft.inventoryByCategory.fish[caughtFish.id]) {
            draft.inventoryByCategory.fish[caughtFish.id] = { qty: 0, quality: 'normal', acquiredAt: Date.now() };
          }
        ...
        get().enqueueNotification(`🎣 Kurcaci Mamat mendapat ${caughtFish.emoji} ${caughtFish.name}!`, { id: 'auto-fisher', type: 'success' });
      }
    }
    ...
    if (isWorkerActive(state, 'chef') && state.selectedRecipe) {
      ...
      if (recipe) {
        const typeQueue = craftingQueue.filter((q) => RECIPES.find((r) => r.id === q.recipeId)?.type === recipe.type);
        if (typeQueue.length < 3) {
          ...
          if (canCraft) {
            ...
            queueChanged = true;
            set({ craftingQueue });
            get().enqueueNotification(`👨‍🍳 Kurcaci Juna memasak ${recipe.name}!`, { id: 'auto-chef', type: 'success' });
          }
        }
      }
    }
  ...
  calculateOfflineProgress: () => {
    ...
    if (isWorkerActive(state, 'rancher')) {
      for (let i = 0; i < newAnimals.length; i++) {
        ...
        if (data && a.status === 'producing') {
          ...
          if (cycles > 0) {
            offlineItems.push({ cat: catFor(data.product), id: data.product, qty: cycles });
    ...
    if (isWorkerActive(state, 'fisher')) {
      ...
      const expectedCatches = Math.floor(attempts * GAME_CONSTANTS.CHANCES.AUTO_FISHER_CATCH);
      ...
      set(draft => {
        ...
        for (const item of offlineItems) {
          if (!draft.inventoryByCategory[item.cat][item.id]) {
            draft.inventoryByCategory[item.cat][item.id] = { qty: 0, quality: 'normal', acquiredAt: now };
          }
          draft.inventoryByCategory[item.cat][item.id].qty += item.qty;
        }
        draft.offlineReport = { deltaSeconds, harvestedCrops, collectedProducts, caughtFishes, minedGems, maturedCrops: 0, maturedNodes: 0, earnedCoins };
      });
    } else {
      set({ lastSavedAt: now });
    }
