          ...
          <SideDock
            tabs={[
              { id: 'toko', label: 'Toko', emoji: '🏪', content: <TownShop /> },
              {
                ...
                content: (
                  <>
                    <h3 className="shop-section-title"><span>👥</span> Hubungan NPC</h3>
                    <div className="space-y-2 mb-3">
                      {NPC_LIST.map((npc) => {
                        const state = npcs?.[npc.id] || { level: 1, points: 0, hearts: 1 };
                        const hearts = state.hearts || 1;
                        return (
                          <div key={npc.id} className="glass-card p-2 flex items-center justify-between">
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              <span className="text-lg">{npc.emoji}</span>
                              <div className="min-w-0">
                                <div className="font-bold text-xs text-[var(--text-primary)] truncate">{npc.name}</div>
                                <div className="text-[9px] text-[var(--text-secondary)]">{npc.role}</div>
                                <div className="text-[10px]">
                                  {'❤️'.repeat(Math.min(hearts, 10))}{'🖤'.repeat(Math.max(0, 10 - Math.min(hearts, 10)))}
                                </div>
                              </div>
                            </div>
                            <button
