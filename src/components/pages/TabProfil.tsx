...
function ActionWidget() {
  ...
  if (readyPlots.length > 0) {
    hints.push({
      emoji: "🌾",
      text: `${readyPlots.length} tanaman siap dipanen di Ladang!`,
  ...
  const readyAnimals = animals.filter(
    (a) =>
      a.status === "producing" &&
      now - (a.lastCollected || 0) >= (a.produceTime || 60000),
  );
  if (readyAnimals.length > 0) {
    hints.push({
      emoji: "🐄",
      text: `${readyAnimals.length} hewan siap diambil hasilnya di Ternak!`,
  ...
  if (activeCustomers.length > 0) {
    hints.push({
      emoji: "👥",
      text: `${activeCustomers.length} pelanggan menunggu di Restoran!`,
      color: "bg-orange-100 border-orange-300 text-orange-800",
    });
  }

  // 4. Masakan siap diambil dari dapur?
  const doneCooking = craftingQueue.filter(
    (q) => now - q.startTime >= q.duration,
  );
  if (doneCooking.length > 0) {
    const recipeName =
      RECIPES.find((r) => r.id === doneCooking[0].recipeId)?.name || "Masakan";
    hints.push({
      emoji: "🍳",
      text: `${recipeName} selesai dimasak — ambil di Restoran!`,
  ...
  const urgentOrders = (orders || []).filter((o) => {
    const remainingMs = (o.timer || 0) * 1000 - (now - (o.createdAt || 0));
    return remainingMs > 0 && remainingMs < 5 * 60 * 1000;
  });
  if (urgentOrders.length > 0) {
    hints.push({
      emoji: "⏰",
      text: `${urgentOrders.length} pesanan Order Board segera kadaluarsa!`,
  ...
  if ((inventory.pupuk_kandang || 0) > 0) {
    hints.push({
      emoji: "🌿",
      text: `Punya ${inventory.pupuk_kandang} Pupuk Kandang — otomatis dipakai saat tanam!`,
  ...
  if ((inventory.cacing || 0) >= 2) {
    hints.push({
      emoji: "🪱",
      text: `Punya ${inventory.cacing} Cacing — bisa dibuat Umpan di menu Memancing!`,
    ...
    <div className="glass-panel p-4 mb-5">
      ...
      <div className="space-y-2">
        {hints.map((hint, i) => (
          <div
            key={i}
            className={`flex items-center gap-2 text-xs font-semibold px-3 py-2 rounded-xl border-2 ${hint.color}`}
          >
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-[#FFFDF7] border-4 border-[var(--wood)] rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl relative">
              ...
              <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                {process.env.NODE_ENV === "development" && (
                  <div>
                    <h4 className="text-xs font-black text-red-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                      <Zap className="w-4 h-4" /> Mode Developer (Cheat)
                    </h4>
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        ...
                        onClick={() => {
                          dev.addCoins(10000);
                          enqueueNotification("+10.000 Koin!", {
                            type: "success",
                          });
                        }}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          dev.addEnergy(100);
                          enqueueNotification("+100 Energy!", {
                            type: "success",
                          });
                        }}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          dev.instantGrow();
                          enqueueNotification("Semua tanaman langsung panen!", {
                            type: "success",
                          });
                        }}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          dev.unlockAll();
                          enqueueNotification(
                            "Semua pekerja & bangunan terbuka!",
                            { type: "success" },
                          );
                        }}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          dev.setLevel(50);
                          enqueueNotification("Level Maksimal!", {
                            type: "success",
                          });
                        }}
                      >
                ...
                <div>
                  <h4 className="text-xs font-black text-[var(--text-secondary)] uppercase tracking-wider mb-2">
                    Sistem
                  </h4>
                  <Button
                    variant="danger"
                    ...
                    onClick={() => {
                      ...
                      useGameStore
                        ...
                        .openConfirm(
                          "Reset Game",
                          "Apakah Anda yakin ingin menghapus SEMUA data permainan? Ini tidak bisa dikembalikan!",
                          () => {
                            resetGame();
                            enqueueNotification("Game telah direset!", {
                              type: "success",
                            });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {ACHIEVEMENTS.map((ach) => {
              ...
              return (
                <div
                  key={ach.id}
                  className={`p-3 rounded-2xl border-2 flex items-start gap-3 transition-all ${isUnlocked ? "bg-[#fff7e6] border-[var(--gold)] shadow-sm" : "bg-black/5 border-black/10 opacity-70 grayscale"}`}
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-secondary)] mt-1 leading-snug">
                      {isSecret
                        ? "Selesaikan kondisinya untuk membuka."
                        : ach.desc}
                    </p>
